...
static int bgraToYv12Wrapper(SwsContext *c, const uint8_t *src[],
							 int srcStride[], int srcSliceY, int srcSliceH,
							 uint8_t *dst[], int dstStride[])
{
    ff_rgbatoyv12(
...
static int rgbaToYv12Wrapper(SwsContext *c, const uint8_t *src[],
							 int srcStride[], int srcSliceY, int srcSliceH,
							 uint8_t *dst[], int dstStride[])
{
    ff_bgratoyv12(
...
void ff_get_unscaled_swscale(SwsContext *c)
{
    ...
    if (srcFormat == AV_PIX_FMT_BGRA &&
        (dstFormat == AV_PIX_FMT_YUV420P || dstFormat == AV_PIX_FMT_YUVA420P) &&
        !(flags & SWS_ACCURATE_RND) && !(dstW&1))
        c->convert_unscaled = bgraToYv12Wrapper;

    /* rgb24toYV12 */
    if (srcFormat == AV_PIX_FMT_RGBA &&
        (dstFormat == AV_PIX_FMT_YUV420P || dstFormat == AV_PIX_FMT_YUVA420P) &&
        !(flags & SWS_ACCURATE_RND) && !(dstW&1))
        c->convert_unscaled = rgbaToYv12Wrapper;
